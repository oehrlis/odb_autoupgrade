...
# Date.......: 2026.01.12
# Revision...: 0.3.0
# Purpose....: Development workflow automation for OraDBA AutoUpgrade extension.
...
EXTENSION_NAME := $(shell grep '^name:' .extension 2>/dev/null | awk '{print $$2}' || echo "odb_autoupgrade")
...
help: ## Show this help message
	@echo -e "$(COLOR_BOLD)OraDBA AutoUpgrade Development Makefile$(COLOR_RESET)"
...
lint-markdown: ## Lint Markdown files with markdownlint
	...
	@if [ -n "$(MARKDOWNLINT)" ]; then \
		$(MARKDOWNLINT) --config .markdownlint.yaml '**/*.md' --ignore node_modules --ignore dist --ignore build --ignore CHANGELOG.md --ignore 'doc/release_notes/**' || exit 1; \
...
version: ## Show current version
	@echo -e "$(COLOR_BOLD)OraDBA AutoUpgrade Version: $(COLOR_GREEN)$(VERSION)$(COLOR_RESET)"
...
info: ## Show project information
	@echo -e "$(COLOR_BOLD)OraDBA AutoUpgrade Extension Information$(COLOR_RESET)"
	@echo ""
	...
	@echo ""
	@echo "Directories:"
	@for dir in bin sql rcv etc lib doc jar patches scripts tests; do \
		if [ -d "$$dir" ]; then \
